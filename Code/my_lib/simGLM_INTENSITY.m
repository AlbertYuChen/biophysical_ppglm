function [tsp,sps,Itot,Istm] = simGLM_INTENSITY(glmprs, Stim, rndseed)
...
while jbin <= rlen
    ...
    if (tspnext >= rrcum(end)) % No spike in this window
        ...
        %-------  select which post-history filter should use by searching
        % the pool of all valid ranges vv = {glmprs.valid_point_range}; -------
