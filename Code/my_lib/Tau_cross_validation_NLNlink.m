...
function Tau = Tau_cross_validation_NLNlink(glmprs, stim, spktrMat, KS_valid_point_range, Itot_function)
...
for ti = 1:N_trails
    ...
    ItotNLN = Itot_function(glmprs, stim, spktrMat(ti,:));    
%     validIndex = ItotNLN<=glmprs.BasisEnd & ItotNLN>=glmprs.BasisBegin;
%     ItotBasisIndex = round( (ItotNLN(validIndex) - glmprs.BasisBegin) / glmprs.dx);    
%     ItotNLN(validIndex) = glmprs.f_Link_NLN(ItotBasisIndex);
%     ItotNLN(ItotNLN>glmprs.BasisEnd) = 0;
%     ItotNLN(ItotNLN<glmprs.BasisBegin) = -50;    
