function [tsp,sps,ItotNLN,Istm] = simGLM_NLNlink(glmprs, Stim, rndseed)
...
while jbin <= rlen
    ...
    ItotSeg = Itot(iinxt);    
%     validIndex = ItotSeg<=glmprs.BasisEnd & ItotSeg>=glmprs.BasisBegin;
    ItotSeg(ItotSeg>glmprs.BasisEnd) = 0;
    ItotSeg(ItotSeg<glmprs.BasisBegin) = -30;  
    ItotBasisIndex = round( (ItotSeg - glmprs.BasisBegin) / glmprs.dx);    
    ItotSeg = glmprs.f_Link_NLN(ItotBasisIndex);     
    rrnxt = exp(ItotSeg);
...
ItotBasisIndex = round( (Itot(validIndex) - glmprs.BasisBegin) / glmprs.dx);    
ItotNLN(validIndex) = glmprs.f_Link_NLN(ItotBasisIndex);
